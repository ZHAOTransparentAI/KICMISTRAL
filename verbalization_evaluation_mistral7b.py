...
# Function to clean the prediction text by removing numbers, dots, and newlines
def clean_prediction(pred):
    # Remove patterns like "1. ", "2. " and newline characters
    return re.sub(r'(\d+\.\s*|\n)', '', pred).strip()
...
def calculate_metrics(data):
    ...
    for entry in data:
        ...
        # Use re.split to handle both ',' and '|' as delimiters
        predictions = [clean_prediction(pred.strip()) for pred in re.split(r'[,\|]', entry['Prediction'])]
...
json_file_path = 'outputs/wn18rr/100_test/Efficient_demonstration_steps/output_tail_mistral8x7b_test_50can_eff_demon_step6.txt'
