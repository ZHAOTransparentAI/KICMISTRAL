...
class ChatGPT:
    ...
    def query_API_to_get_message(self, messages):
        while True:
            try:
                ...
                # Extracting role and content from the response
                role = res.choices[0].message.role  #role
                content = res.choices[0].message.content  # content
    ...
    def load_prompt_template(self, prompt_path, prompt_name):
        if prompt_path.endswith(".json"):
            with open(prompt_path, "r", encoding=self.detect_file_encoding(prompt_path)) as f:
                prompt = json.load(f)
...
class Solver:
    def __init__(self, args):
        ...
        # self.tokenizer = AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", use_auth_token=args.api_key)  # Use the appropriate tokenizer
    ...
    def parse_result(self, response, parse_type):
        ...
        if parse_type == "final_answer":
            final_order = ""  # Initialize with a default value
            if "the final order:" in response:
                ...
                print(f"Expected 'the final order:' not found in response: {response}")
        return final_order
    ...
    def load_all_candidate_answers(self):
        with open("dataset/" + self.args.dataset + "/retriever_candidate_"+ args.query +".txt",'r') as load_f:
            self.all_candidate_answers=json.load(load_f) # How to get this txt file
    ...
    def load_rel_txt_to_id(self):
        file_path = 'dataset/' + self.args.dataset + '/get_neighbor/relation2id.txt'
        encoding = self.LLM.detect_file_encoding(file_path)
    ...
    def load_ent_map_id(self):
        file_path = 'dataset/' + self.args.dataset + '/get_neighbor/entity2id.txt'
        encoding = self.LLM.detect_file_encoding(file_path)
        with open(file_path, 'r', encoding=encoding) as file:
    ...
    def load_ent_to_text(self):
        file_path = 'dataset/' + self.args.dataset + '/entity2text.txt'
...
def parse_args():
    ...
    parser.add_argument('--candidate_num', default=30, type=int) #default=50
    ...
    parser.add_argument('--api_key', default="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", type=str)
    ...
    args.output_path = './outputs/'+ args.dataset +'/output_'+ args.query +'_mistral8x7b_test_30can.txt'
    args.chat_log_path = './outputs/'+ args.dataset +'/chat_'+ args.query +'_mistral8x7b_test_30can.txt'
    # Validate and prepare API keys
    if args.api_key.startswith("hf_"):
        all_keys = [args.api_key]  # Single API key provided
    else:
    ...
    return args, all_keys

if __name__ == '__main__':
    args, all_keys = parse_args()

    test_triplet = []
    with open("dataset/" + args.dataset + "/test_answer_100.txt",'r') as load_f:
